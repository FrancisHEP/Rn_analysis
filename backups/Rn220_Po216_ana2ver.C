...
void Rn220_Po216(int &RunNo)
{
  ...
  double ERn, EPo;
  TFile *outFile = new TFile(Form("new_aa_tree/tree_Th_aa_run%d.root",RunNo),"recreate");
  ...
  for (int ii = 0; ii < totalEvt; ii++){
    ...
    // modified by Wenbo
    // 2018-11-23
    // nan values for xS2LRF 
    if(xS2LRF[s2max]!=xS2LRF[s2max]||yS2LRF[s2max]!=yS2LRF[s2max]) continue;
    */
