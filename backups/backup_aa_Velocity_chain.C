...
void aa_Velocity_chain() // (int& RunNo)
{
  // initializing...
  TChain *ch;
  ch = new TChain("aa_tree");
  // ch->Add(Form("tree_Th_aa_run%d.root",RunNo));
  vector<int> vetorunvec; SetRunList("/home/mawenbo/radonRun/Rn_analysis/Run13Rn_run_part.lst", vetorunvec);
  for (int i=0;i<vetorunvec.size();++i) {
    ch->Add(Form("/home/mawenbo/radonRun/Rn_analysis/rootfile_aa_tree/tree_Th_aa_run%d.root",
		 vetorunvec[i]));
    printf("loading...%d\n",vetorunvec[i]);
  }
  // main...
  ...
  for(int j = 0 ; j < totalEvt; j++){
    ...
    if(layer_z>=12) layer_z=12-1; // modified. some bugs here! some 
    ...
    v_zx_z[index_x][index_z] += v_z; // bugs
    N_yx[layer_z][index_x][index_y] += 1; // bugs
...
// ifstream runlistfile("/home/mawenbo/radonRun/Run13Rn_run.lst");
// if (path_file.good()) {
//   int current_number = 0;
//   while (path_file >> current_number){
//     path_file.push_back(current_number);
//   }
//   path_file.close();
// } else {
//   cout << "Error! No run list found";
