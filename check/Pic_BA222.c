...
void Pic_BA222(){
	...
	chain->Add("outTree_222_BA_run10.root");
  ...
  gStyle->SetNumberContours(100);
	...
	TF1 *f2=new TF1("f2","[0]*exp(-x/[1])",10,500);
	f2->SetParameters(counts/20,237);
	h2->Fit("f2","R");

	TLatex *t1=new TLatex(200,h2->GetMaximum()*0.5,Form("Dt=%.2f#pm%.2f#mus",f2->GetParameter(1),f2->GetParError(1)));
	t1->SetTextColor(2);
	t1->SetTextSize(0.04);
	t1->Draw("same");
	TLatex *t2=new TLatex(200,h2->GetMaximum()*0.7,Form("%.2fmBq in FV",counts/0.8614/Time*1000));
        t2->SetTextColor(2);
        t2->SetTextSize(0.04);
        t2->Draw("same");

c1->SaveAs("Rn222cuts.pdf");
c2->SaveAs("Rn222DecayTime.pdf");
c3->SaveAs("Rn222S1S2.pdf");
}
