...
void Pic_BiPo212(int runnumber){

  //gStyle->SetPalette(kBird);
  gStyle->SetNumberContours(100);
	    TChain *chain;
    chain = new TChain("rn220_BA");
	chain->Add(Form("outTree_212_BiPo_%d.root",runnumber));
	...
	TF1 *f2=new TF1("f2","[0]*exp(-x/[1])",0.3,2);
	...
	chain->Draw("yS2PAF[s2max]:xS2PAF[s2max]>>hxy",,FV&&R2&&Cut3&&s1z&&s2z&&"(tS1[alpha]-tS1[beta])/1000>0&&(tS1[alpha]-tS1[beta])/1000<3","colz");
