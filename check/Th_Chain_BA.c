...
void Th_Chain_BA(int runnumber)
{
...
//        for(std::string line; std::getline(inf,line);) chain->Add(line.c_str());
	//chain.Add(Form("/nas/64-core/%d/ana*",runnumber));
	chain.Add(Form("/nas/a/zhangdan/anaPAF/%d/ana*",runnumber));
    int runNo, trigNo;
    double triggerTime;
  ...
  int beta, alpha;
  TFile *outFile = new TFile(Form("outTree_212_BiPo_%d.root",runnumber),"recreate");
  TTree *outTree = new TTree("rn220_BA","radon220_BA");
...
//    ly = 4.4/1.1732;
        ...
        double ly_Beta_3000 = 4.315*0.755/1.063;        // just the same, using 164kev    

    double lowAlpha = 3000*ly_Alpha;
    ...
    if(nS2<1) continue;
      ...
      for(int j=0; j<nS1; j++){
          if(qS1[j]<highBeta&&qS1[j]>lowBeta){
//          cout<<j<<endl<<qS1[j]<<endl;
            Beta[nBeta] = j;
            nBeta++;
          }
