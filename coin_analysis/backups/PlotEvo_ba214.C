...
void PlotEvo_ba214(const char * inputfilename1, const char * inputfilename2)
{
  ...
  if (fin2.good()) {
    ...
    while(fin2>>t1>>t2)
      {
	runno2[i]=t1;
	ba214err[i]=sqrt(t2)/duration[i];
	ba214[i]=t2/duration[i];
	i++;
      }
    fin2.close();
    fin2line = i;
  ...
  TGraph * g1;
  g1 = new TGraphErrors(i,time,ba214,duration,ba214err);  // x y xerr yerr
  g1->SetTitle("");
  ...
  g1->Draw("");
  c1->SaveAs("evo_ba214_2018.pdf");
  c1->SaveAs("evo_ba214_2018.root");
