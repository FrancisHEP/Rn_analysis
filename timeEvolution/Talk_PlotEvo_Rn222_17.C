...
// daqfile, rnEvofile
// void PlotRnEvo2018()
void Talk_PlotEvo_Rn222_17(const char * inputfilename1, const char * inputfilename2)
{
  ...
  g1->GetYaxis()->SetLabelSize(0.032);
  g1->GetYaxis()->SetTitleOffset(0.82);
  g1->GetYaxis()->CenterTitle();
  g1->GetYaxis()->SetTitle("Activity [Bq]");
  g1->GetYaxis()->SetTickLength(0.01);
  g1->GetYaxis()->SetRangeUser(0.034,0.05);
  g1->Draw("AP");
  // int xmin = TDatime(2017,11,09,00,00,00).Convert(); // 60days
  int xmin = TDatime(2017,12,21,12,00,00).Convert(); // 60days
  int xmax = TDatime(2018,01,10,12,00,00).Convert(); // 01-15
  g1->GetXaxis()->SetRangeUser(xmin,xmax);
  //  g1->GetYaxis()->SetRangeUser(1e-2,3e-1);
  //  c1->SetLogy();

  int t0 = time[81]; printf("t0 %d\n",t0);
  for (int i=0;i<136;++i) {time[i]-=t0; time[i]/=86400; duration[i]/=86400;}
