...
void BiPo_coin(int &RunNo)
{
  ...
  for(int i=0; i<totalEvt; i++){
    ...
    for(int j = 0; j < nS1; j++){
      if (s1max<0) continue;
      if(qS1[j]>lowAlpha && hS1[j]>2000 && ttenS1[j]+wtenS1[j]-pS1[j]<240 && 
	 ttenS1[j]+wtenS1[j]-pS1[j]>=90 && hS1[j]/qS1[j]>0.12){
	alpha = j;
      for(int k = 0; k < j; k++){
	if(qS1[k]<highBeta2 && qS1[k]>lowBeta2 && tS1[alpha]-tS1[k]>t_low2 && 
	   tS1[alpha]-tS1[k]<t_high2 && tS1[k]<tS2[nS2-1] && qBeta2<qS1[k]) {
	  qBeta2 = qS1[k];
	  beta = k;
	}
      }
